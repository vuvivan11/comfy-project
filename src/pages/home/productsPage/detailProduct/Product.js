...
export default function Product(props) {
    ...
    const renderColor = (colors) => {
        return colors.map((color, index) => {
            return <span key={index} className="product-color" style={{ backgroundColor: `${color}` }} />
        })
    }
                    ...
                    <article key={price} className="product">
                        <div className="product-container">
                            ...
                            <div className="product-icons">
                                <Link to={`${product.id}`} className="product-icon" onClick={() => {
            ...
            <>
                <section className="single-product">
                    <div className="section-center single-product-center">
                        <img src={`${image[0].thumbnails.large.url}`} className="single-product-img img" alt={`${image[0].filename}`} />
                        <article className="single-product-info">
                            <div>
                                ...
                                <button className="addToCartBtn btn" onClick={() => { handleAddCart(detailProduct) }}>
                                    add to cart
                                </button>
                            </div>
                        </article>
